...
for i in entityname:
  ...
  mae = pd.DataFrame(index=np.arange(weeks_of_pred), columns=np.arange(2*final_step+2))
  mae_col_name = []
  ...
  for t in range(6, -1, -1):
      ...
      week0 = entity1.iloc[len(entity1)-1*48*(3*7+t):len(entity1)-48*(t+7), col_num:col_num+final_step]
      m = week0.mean()
      ...
      naive.columns = name
      naive.to_excel('/content/drive/MyDrive/BE/BE_'+str(energy_type)+'_New/naive/past2weeksmean/'+str(save_names)+'/naive_predictions/'+str(i)+' prediction at '+str(6-t)+'day.xlsx')
  ...
  mae.to_excel('/content/drive/MyDrive/BE/BE_'+str(energy_type)+'_New/naive/past2weeksmean/'+str(save_names)+'/naive_error/'+str(i)+'.xlsx')
  print(i)
  g+=1
  tot_pred +=1
...
tot_error.to_excel('/content/drive/MyDrive/BE/BE_'+str(energy_type)+'_New/naive/past2weeksmean/'+str(save_names)+'/2weekback.xlsx')
